...
void CPU::nmi(){
    ...
    //qDebug() << "NMI, reg_pc = " << reg_pc << Qt::endl;
    cycles_wait = 7;
}
...
int CPU::XXX(){
    qDebug() << "执行到了尚未定义的命令: " << opcode << Qt::endl;
    abort();
    return 0;
}
...
void CPU::print_log() const
{
    ...
    qDebug() << "Program Counter = " << reg_pc << ", opcode = " << opcode << ", code = " << this->inst_table[opcode].name.c_str() << addr_str << ", A = " << this->reg_a << ", X = " << this->reg_x << ", Y = " << this->reg_y << ", stack pointer = " << reg_sp << ", NVDIZC = " << int(reg_sf.get_n()) << int(reg_sf.get_v()) << int(reg_sf.get_d()) << int(reg_sf.get_i()) << int(reg_sf.get_z()) << int(reg_sf.get_c()) << Qt::endl;
    //qDebug() << "Program Counter = " << reg_pc << ", opcode = " << opcode << ", code = " << this->inst_table[opcode].name.c_str() << addr_str << Qt::endl;
}
